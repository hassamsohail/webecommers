...
function FaceWash() {
  ...
  const Delcte = (item) => {
    //   console.log(item.id);
    //  collection(db,"FaceWash").where(item.id).then((documentSnapshot)=>{
    //   console.log(documentSnapshot.data());
    //  })
    const docref = doc(db, 'FaceWash', item.id)
    deleteDoc(docref)
      .then(() => {
        GetDoc()
  ...
  const GetDoc = () => {
    getDocs(collection(db, 'FaceWash')).then((querySnapshot) => {
      ...
      querySnapshot.forEach((documentSnapshot) => {
        ...
        SetFaceWashData((FaceWashData) => [...FaceWashData, d])
      })
    })
  }
      ...
      .then(() => {
        getDownloadURL(imageRef)
          .then(async (url) => {
            setState1(url)
            try {
              await addDoc(collection(db, 'FaceWash'), {
