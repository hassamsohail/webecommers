...
function HairOils() {
  ...
  const [HairOilsData, SetHairOilsData] = useState([])
  ...
  const Delcte = (item) => {
    //   console.log(item.id);
    //  collection(db,"HairOils").where(item.id).then((documentSnapshot)=>{
    //   console.log(documentSnapshot.data());
    //  })
    const docref = doc(db, 'HairOils', item.id)
    deleteDoc(docref)
      .then(() => {
        GetDoc()
  ...
  const GetDoc = () => {
    getDocs(collection(db, 'HairOils')).then((querySnapshot) => {
      ...
      querySnapshot.forEach((documentSnapshot) => {
        ...
        SetHairOilsData((HairOilsData) => [...HairOilsData, d])
      })
    })
  }
      ...
      .then(() => {
        getDownloadURL(imageRef)
          .then(async (url) => {
            setState1(url)
            try {
              await addDoc(collection(db, 'HairOils'), {
