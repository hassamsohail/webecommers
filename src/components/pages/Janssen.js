...
function Janssen() {
  ...
  const [JanssenData, SetJanssenData] = useState([])
  ...
  const Delcte = (item) => {
    //   console.log(item.id);
    //  collection(db,"Janssen").where(item.id).then((documentSnapshot)=>{
    //   console.log(documentSnapshot.data());
    //  })
    const docref = doc(db, 'Janssen', item.id)
    deleteDoc(docref)
      .then(() => {
        GetDoc()
  ...
  const GetDoc = () => {
    getDocs(collection(db, 'Janssen')).then((querySnapshot) => {
      ...
      querySnapshot.forEach((documentSnapshot) => {
        ...
        SetJanssenData((JanssenData) => [...JanssenData, d])
      })
    })
  }
      ...
      .then(() => {
        getDownloadURL(imageRef)
          .then(async (url) => {
            setState1(url)
            try {
              await addDoc(collection(db, 'Janssen'), {
