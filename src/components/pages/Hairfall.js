...
function Hairfall() {
  ...
  const [HairfallData, SetHairfallData] = useState([])
  ...
  const Delcte = (item) => {
    //   console.log(item.id);
    //  collection(db,"Hairfall").where(item.id).then((documentSnapshot)=>{
    //   console.log(documentSnapshot.data());
    //  })
    const docref = doc(db, 'Hairfall', item.id)
    deleteDoc(docref)
      .then(() => {
        GetDoc()
  ...
  const GetDoc = () => {
    getDocs(collection(db, 'Hairfall')).then((querySnapshot) => {
      ...
      querySnapshot.forEach((documentSnapshot) => {
        ...
        SetHairfallData((HairfallData) => [...HairfallData, d])
      })
    })
  }
      ...
      .then(() => {
        getDownloadURL(imageRef)
          .then(async (url) => {
            setState1(url)
            try {
              await addDoc(collection(db, 'Hairfall'), {
