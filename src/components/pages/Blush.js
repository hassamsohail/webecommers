...
function Blush() {
  ...
  const Delcte = (item) => {
    //   console.log(item.id);
    //  collection(db,"Blush").where(item.id).then((documentSnapshot)=>{
    //   console.log(documentSnapshot.data());
    //  })
    const docref = doc(db, 'Blush', item.id)
    deleteDoc(docref)
      .then(() => {
        GetDoc()
  ...
  const GetDoc = () => {
    getDocs(collection(db, 'Blush')).then((querySnapshot) => {
      ...
      querySnapshot.forEach((documentSnapshot) => {
        ...
        SetBlushData((BlushData) => [...BlushData, d])
      })
    })
  }
      ...
      .then(() => {
        getDownloadURL(imageRef)
          .then(async (url) => {
            setState1(url)
            try {
              await addDoc(collection(db, 'Blush'), {
